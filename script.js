...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwFA9R7msn9Du9heeJ3r2Ll2SpONwU1S8IF8-tiN1lCgBf6WDTSgMFY6COd4V8IxD5pXQ/exec";
...
async function loadOrderStatus() {
    ...
    try {
        ...
        if (allCompleted) {
            closeBtn.innerHTML = "💰 Sudah Bayar & Tutup Bill";
            closeBtn.disabled = false;
            closeBtn.style.background = "linear-gradient(135deg, var(--green), var(--green2))";
            closeBtn.style.cursor = "pointer";
        } else {
            closeBtn.innerHTML = "⏳ Menunggu pesanan selesai...";
...
async function confirmPaymentAndClose() {
    ...
    try {
        ...
        const allCompleted = order.items.every(item => item.status === "Selesai");
        if (!allCompleted) {
            showShareToast("Pesanan belum selesai semua. Tunggu hingga selesai.");
            return;
        }
        ...
        showShareToast("Error konfirmasi pembayaran");
    }
}
