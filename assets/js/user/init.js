...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // === FIX MOBILE: Tampilkan skeleton dulu ===
    showSkeletonLoading();

    // === FIX: Load menu (sudah ada fallback default di dalam fungsi ini) ===
    await loadMenuFromSheet();

    // === FIX: Pastikan menuData ada isinya sebelum render ===
    const total = getAllItems().length;
    if (total === 0) {
        // Safety net terakhir jika semua gagal
        setDefaultMenu();
    }
